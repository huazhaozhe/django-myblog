...
class MyCheckboxInput(forms.CheckboxInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        html = format_html(
            '<p><label><input{} /><span>' + self.span + '</span></label></p>',
            flatatt(final_attrs))
        return html
...
class PostForm(forms.ModelForm):
    author = forms.ModelChoiceField(
        queryset=User.objects.filter(is_superuser=True), empty_label=None)
    ...
    visible = forms.BooleanField(widget=MyCheckboxInput(span='可见'),
                                 required=False)
    comment_enabled = forms.BooleanField(widget=MyCheckboxInput(span='评论'),
                                         required=False)
