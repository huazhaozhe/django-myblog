...
def github_check(request):
    ...
    try:
        ...
        user = User(username='github'+str(openid), nickname=nickname, email=email, avatar_url=picture)
        user.save()
        ...
        oauth_user = OauthUser(user=user, openid=openid, nickname=nickname, email=email, picture=picture, token=token, type=type)
        oauth_user.save()
...
def weibo_login(request):
    weibo_oauth = WeiboOauth(settings.OAUTH['weibo']['client_id'], settings.OAUTH['weibo']['client_secret'], settings.OAUTH['weibo']['redirect_url'])
...
def weibo_check(request):
    ...
    user = User(username='weibo'+str(openid), nickname=nickname, email=email, avatar_url=picture)
    user.save()
    user.user_permissions.add(Permission.objects.get(codename='add_comment'))
    user.user_permissions.add(Permission.objects.get(codename='add_note'))
    user = User.objects.get(pk=user.pk)
